...
module.exports = {
	...
	async execute(message) {
        ...
        const authorpic = await Canvas.loadImage(message.author.displayAvatarURL({ format: 'jpg' }));
        ctx.drawImage(authorpic, 150, 75, 150, 150);
        const memberpic = await Canvas.loadImage(member.user.displayAvatarURL({ format: 'jpg' }));
        ctx.drawImage(memberpic, 450, 250, 150, 150);
        const attachment = new MessageAttachment(canvas.toBuffer(), 'bonk.jpeg');
        if (member.user.id == message.author.id) return message.channel.send('Holy crap, if you\'re willing to bonk yourself I have no idea what\'s going on in your head.', attachment);
